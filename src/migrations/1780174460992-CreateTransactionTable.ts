...
export class CreateTransactionTable1780174460992 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."transactions_category_enum" AS ENUM('rent', 'Shopping', 'trip', 'grocery', 'uber', 'eating out', 'phone bill', 'wifi', 'subscription', 'electricity', 'gas', 'personal', 'transit', 'one time', 'gift', 'w Raima', 'Doctor', 'Car', 'Gas')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_subcategory_enum" AS ENUM('Eating out', 'Experience', 'Grocery', 'Uber', 'Flight', 'Transit', 'Other', 'Souvenir')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_source_enum" AS ENUM('Chase CC', 'Chase checking', 'Amazon Card', 'Robinhood', 'Bilt', 'Discover', 'Apple')`);
